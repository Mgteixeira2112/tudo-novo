...
s = s.replace("app.use(express.json());", "app.use(express.json({ limit: '64kb' }));")
...
'''
if 'function sanitizePublicReservation' not in s:
    if anchor not in s:
        raise SystemExit('Auth middleware anchor not found')
    s = s.replace(anchor, helpers + anchor)

old_res = """app.post('/api/reservations', (req: Request, res: Response) => {
  try {
...
new_res = """app.post('/api/reservations', publicReservationLimiter, (req: Request, res: Response) => {
  try {
...
elif new_res not in s:
    raise SystemExit('Reservation endpoint anchor not found')

old_order = """app.post('/api/kitchen/orders', (req: Request, res: Response) => {
  try {
...
new_order = """app.post('/api/kitchen/orders', publicRoomServiceLimiter, (req: Request, res: Response) => {
  try {
...
if old_order in s:
    ...
    raise SystemExit('Kitchen order endpoint anchor not found')

p.write_text(s)
Path('docs/FASE-01-06-PUBLIC-ENDPOINT-HARDENING.md').write_text("""# FASE 1.6 — Endpoints públicos endurecidos
