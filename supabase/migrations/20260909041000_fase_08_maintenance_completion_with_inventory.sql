create or replace function public.complete_maintenance_task_atomic(
  p_task_id text,
...
begin
  ...
  if not found or not (
    ...
    raise exception 'Permissão insuficiente para concluir tarefa de Manutenção com consumo.';
  end if;
  ...
  if v_task.sector <> 'Manutencao' then
    raise exception 'Esta operação é exclusiva para tarefas de Manutenção.';
  end if;
  ...
  loop
    ...
    if v_item.sector <> 'Manutencao' then
      raise exception 'O item % não pertence ao estoque de Manutenção.', v_item.name;
    end if;
  ...
  loop
    ...
    insert into public.stock_movements(
      ...
      v_task.id,
      v_operator,
      'Consumo de material na conclusão de tarefa de Manutenção'
    );
  end loop;
...
revoke all on function public.complete_maintenance_task_atomic(text, jsonb, text) from public;
revoke all on function public.complete_maintenance_task_atomic(text, jsonb, text) from anon;
grant execute on function public.complete_maintenance_task_atomic(text, jsonb, text) to authenticated;
