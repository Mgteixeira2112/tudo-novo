...
begin
  ...
  if coalesce(p_deposit_amount, 0) > 0
     and coalesce(nullif(trim(p_payment_method), ''), '') not in ('PIX','Cartao_Credito','Cartao_Debito','Dinheiro') then
    raise exception 'Selecione uma forma de pagamento válida para o valor recebido no Walk-in.' using errcode = '22023';
  end if;
  ...
  if coalesce(p_deposit_amount, 0) > v_total then
    raise exception 'O pagamento no check-in (%) não pode superar o total da hospedagem (%).', p_deposit_amount, v_total using errcode = '22023';
  end if;
  ...
  insert into public.reservations(
    ...
    case when coalesce(p_deposit_amount,0) > 0 then p_payment_method else null end,
    v_note, now(), now()
