alter table public.room_amenity_kit_items
  add column if not exists quantity_basis text not null default 'fixed_per_room';

alter table public.room_amenity_kit_items
  drop constraint if exists room_amenity_kit_items_quantity_basis_check;

alter table public.room_amenity_kit_items
  add constraint room_amenity_kit_items_quantity_basis_check
  check (quantity_basis in ('per_guest', 'fixed_per_room'));

comment on column public.room_amenity_kit_items.quantity_basis is
  'Forma de cálculo da quantidade esperada: per_guest multiplica pela ocupação da reserva; fixed_per_room usa a quantidade uma única vez por quarto.';
...
begin
  ...
  loop
    ...
    v_quantity_basis := coalesce(nullif(v_item->>'quantityBasis', ''), 'fixed_per_room');
    if v_quantity_basis not in ('per_guest', 'fixed_per_room') then
      raise exception 'Forma de cálculo inválida para amenity.';
    end if;
  ...
  for v_item in select value from jsonb_array_elements(p_items)
  loop
    insert into public.room_amenity_kit_items(kit_id, inventory_item_id, quantity, quantity_basis)
    values (
      v_kit.id,
      v_item->>'inventoryItemId',
      (v_item->>'quantity')::numeric,
      coalesce(nullif(v_item->>'quantityBasis', ''), 'fixed_per_room')
    );
  end loop;
...
comment on function public.save_room_amenity_kit_atomic(text, jsonb, text) is
  'Configura amenities por tipo de quarto com quantidade fixa por quarto ou por hóspede. Não movimenta estoque.';
