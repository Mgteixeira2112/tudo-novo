...
begin
  ...
  from public.rooms
  where rooms.id = v_res.room_id;

  if not found then
    raise exception 'Quarto atual não encontrado.';
  end if;
  ...
  where r.id <> v_current_room.id
    and r.type_id = v_current_room.type_id
    and r.status = 'Disponivel'
    and r.capacity >= (coalesce(v_res.adults, 0) + coalesce(v_res.children, 0))
    and not exists (
      select 1
...
revoke all on function public.find_overdue_stay_transfer_rooms(text,date) from public;
grant execute on function public.find_overdue_stay_transfer_rooms(text,date) to authenticated;
...
begin
  ...
  if v_new_room.type_id is distinct from v_old_room.type_id then
    raise exception 'O novo quarto deve ser do mesmo tipo de acomodação nesta etapa.';
  end if;
  if v_new_room.capacity < (coalesce(v_res.adults, 0) + coalesce(v_res.children, 0)) then
    raise exception 'O novo quarto não comporta a ocupação atual.';
  end if;
  if v_new_room.status <> 'Disponivel' then
    raise exception 'O quarto % não está disponível para transferência.', v_new_room.number;
  end if;
  ...
  v_note := concat(
    'Transferência do quarto ', v_old_room.number,
    ' para o quarto ', v_new_room.number,
    ' com prorrogação até ', to_char(p_new_check_out_date, 'DD/MM/YYYY'),
  ...
  set status = 'Ocupado',
      ...
      notes = 'Hóspede transferido do quarto ' || v_old_room.number || '. Saída prevista em ' || to_char(p_new_check_out_date, 'DD/MM/YYYY') || '.'
  ...
  set room_number = v_new_room.number,
      updated_at = now()
  where related_type = 'Reserva'
    and related_id = v_res.id
...
revoke all on function public.extend_overdue_stay_with_transfer_atomic(text,date,text,text) from public;
grant execute on function public.extend_overdue_stay_with_transfer_atomic(text,date,text,text) to authenticated;
