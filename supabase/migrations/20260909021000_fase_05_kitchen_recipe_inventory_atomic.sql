-- FASE 5 — baixa automática dos ingredientes no pedido de cozinha
-- Evolui create_kitchen_order_atomic preservando pedido + financeiro + estoque em uma única transação.
...
begin
  ...
  if p_destination not in ('Quarto','Restaurante','Piscina') then
    raise exception 'Destino inválido.';
  end if;

  if p_delivery_sector not in ('Cozinha','Room Service') then
    raise exception 'Setor de entrega inválido.';
  end if;
  ...
  -- Valida e normaliza todos os itens antes de qualquer baixa.
  ...
  loop
    ...
    where id = v_entry->>'menuItemId' and available = true;

    if not found then
      raise exception 'Item de cardápio inválido ou indisponível.';
    end if;
    ...
    if v_menu.operational_type = 'recipe'
       and not exists (
         select 1 from public.menu_item_ingredients mi where mi.menu_item_id = v_menu.id
       ) then
  ...
  -- Consolida a necessidade do pedido inteiro. Um mesmo ingrediente usado em
  -- várias receitas é validado e baixado uma única vez pela quantidade total.
  for v_req in
    with order_items as (
      select
        e.value->>'menuItemId' as menu_item_id,
        (e.value->>'quantity')::numeric as sold_qty
      from jsonb_array_elements(p_items) e(value)
    ), raw_requirements as (
      select
        m.simple_inventory_item_id as inventory_item_id,
  ...
  loop
    ...
    if not found then
      raise exception 'Produto de estoque vinculado não encontrado.';
    end if;
    ...
    insert into public.stock_movements(
      ...
      v_next,
      v_inv.cost_price,
      v_inv.cost_price * v_req.required_qty,
      v_room.number,
      v_res.id,
      v_order_id,
