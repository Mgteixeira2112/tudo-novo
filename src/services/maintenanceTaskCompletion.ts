...
export interface MaintenanceMaterialConsumption {
...
export async function completeMaintenanceTaskAtomic(
  taskId: string,
  materials: MaintenanceMaterialConsumption[] = []
): Promise<KanbanTask> {
  ...
  const { data, error } = await supabase.rpc('complete_maintenance_task_atomic', {
    p_task_id: taskId,
