...
import { RoomReservationOperationalModal } from './RoomReservationOperationalModal.tsx';
...
function RoomsKanbanView({
  ...
  return (
    <div className="space-y-5">
          ...
          return (
            <section key={column.status} className="min-w-0 self-start rounded-2xl border border-[#E6E3D8] bg-[#F8F7F2] p-3">
              <header className="mb-3 flex items-start justify-between gap-2 px-1 pt-1">
                ...
                <span className="min-w-6 rounded-full bg-white px-2 py-1 text-center text-[10px] font-black text-[#3A5A40] shadow-sm">{columnRooms.length}</span>
              </header>
              ...
              <div className={visibleColumns.length === 1
                ...
                {columnRooms.length === 0 && (
                  <div className="col-span-full rounded-xl border border-dashed border-[#DADFD1] bg-white/60 px-3 py-6 text-center text-[10px] text-[#8E9280]">Nenhum quarto neste status</div>
                )}
              </div>
            </section>
          );
      ...
      {selectedRoom && (
        <RoomReservationOperationalModal
          room={selectedRoom}
          canManage={canManage}
          busy={busyRoomId === selectedRoom.id}
          safeTargets={SAFE_TARGETS[selectedRoom.status]}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
      <div className="flex flex-col gap-4 rounded-2xl border border-[#E6E3D8] bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div><h2 className="text-xl font-black tracking-tight text-[#2C3327]">Operação Hoteleira</h2></div>
        ...
        <div className="inline-flex max-w-full overflow-x-auto rounded-xl border border-[#E6E3D8] bg-[#F8F7F2] p-1">
          ...
          >
            <BedDouble className="h-4 w-4" /> Quartos Geral
          ...
          >
            <ClipboardList className="h-4 w-4" /> Tarefas Geral
          ...
          >
            <Sparkles className="h-4 w-4" /> Governança
      ...
      {view === 'rooms' && <RoomsKanbanView key={`rooms-${navigationVersion}`} initialStatus={initialRoomStatus} initialSearch={initialRoomSearch} />}
      {view === 'tasks' && <KanbanBoard key={`tasks-${navigationVersion}`} initialSector={initialTaskSector || 'Todos'} />}

      {view === 'housekeeping' && (
        <div className="space-y-2">
          <div className="rounded-2xl border border-[#CCD5AE] bg-[#F8FAF2] p-5">
            <div className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-[#588157]" /><h3 className="text-lg font-black text-[#2C3327]">Governança</h3></div>
          </div>
      ...
      {view === 'maintenance' && (
        <div className="space-y-2">
          <div className="rounded-2xl border border-[#D4A373]/40 bg-[#FAEDCD]/30 p-5">
            <div className="flex items-center gap-2"><Wrench className="h-5 w-5 text-[#BC6C25]" /><h3 className="text-lg font-black text-[#2C3327]">Manutenção</h3></div>
          </div>
