...
export const HotelSettingsModal: React.FC<HotelSettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm overflow-hidden animate-fade-in">
      <div className="bg-white rounded-2xl sm:rounded-3xl max-w-3xl w-full max-h-[calc(100vh-1.5rem)] sm:max-h-[calc(100vh-2rem)] shadow-2xl border border-[#E6E3D8] flex flex-col overflow-hidden">
        {/* Modal Header */}
        <div className="shrink-0 flex items-center justify-between px-5 sm:px-8 pt-5 sm:pt-8 pb-4 border-b border-[#E6E3D8] bg-white">
          <div className="flex items-center space-x-2.5 min-w-0 pr-3">
            <div className="p-2.5 bg-[#F4F1EA] rounded-xl text-[#2C3327] shrink-0">
          ...
          <button
            onClick={onClose}
            className="text-[#8E9280] hover:text-[#2C3327] p-2 rounded-lg hover:bg-[#F4F1EA] shrink-0"
          >
            ✕
          </button>
        </div>
          ...
          {activeTab === 'visual' && (
            <form onSubmit={handleSaveSettings} className="space-y-4 pt-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-[#6B705C] mb-1">Moeda</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#6B705C] mb-1">Taxa de Serviço (%)</label>
                  <input
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#6B705C] mb-1">Check-in Padrão</label>
                  <input
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#6B705C] mb-1">Check-out Padrão</label>
                  <input
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#6B705C] mb-1">E-mail de Reservas</label>
                  <input
                    id="input-cfg-email"
                    type="email"
                    value={email}
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#6B705C] mb-1">Senha do Wi-Fi dos Hóspedes</label>
                  <input
                    id="input-cfg-wifi"
                    type="text"
                    value={wifiPassword}
              ...
              <div>
                <label className="block text-xs font-semibold text-[#6B705C] mb-1">Políticas de Reserva & Cancelamento</label>
                <textarea
            ...
            <div className="space-y-4 pt-4">
              ...
              <div className="space-y-3">
                {roomTypes.map(rt => (
                  <div
                    key={rt.id}
                    className="p-4 bg-[#F4F1EA] rounded-2xl border border-[#E6E3D8] grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_auto] gap-4 sm:items-center"
                  >
                    <div className="space-y-1 min-w-0">
                      <h5 className="font-bold text-sm text-[#2C3327]">{rt.name}</h5>
                      <p className="text-xs text-[#6B705C] sm:line-clamp-2">{rt.description}</p>
                    ...
                    <label className="flex items-center justify-between sm:justify-end gap-2 shrink-0 rounded-xl bg-white/70 sm:bg-transparent p-2 sm:p-0">
                      <span className="text-xs text-[#6B705C] whitespace-nowrap">Diária Base: {currency}</span>
                      <input
                        type="number"
                        min="50"
                        step="10"
              ...
              <div className="sticky bottom-0 -mx-5 sm:-mx-8 px-5 sm:px-8 py-3 border-t border-[#E6E3D8] bg-white/95 backdrop-blur flex justify-end">
                <button
              ...
              <div className="p-4 bg-[#2C3327] text-[#FDFBF7] rounded-2xl space-y-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="flex items-center space-x-1.5 px-3 py-1 bg-[#588157] hover:bg-[#3A5A40] text-[#FDFBF7] rounded-lg text-xs font-bold transition shrink-0"
                  >
                    {copiedSql ? (
                      <>
