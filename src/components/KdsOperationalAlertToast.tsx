...
import { BellRing, Database, Volume2, X } from 'lucide-react';
import {
  getPublicKdsOperationalAlerts,
  PublicKdsOperationalAlert
} from '../services/kdsOperationalAlerts.ts';
import { enableOldHotelBell, playOldHotelBell } from '../services/alertBell.ts';
...
export const KdsOperationalAlertToast: React.FC<{ token: string }> = ({ token }) => {
  ...
  const handleEnableSound = async () => {
    ...
    const ok = await enableOldHotelBell();
    setSoundEnabled(ok);
  ...
  return (
    <>
      {!soundEnabled && (
        <div className="fixed bottom-5 left-5 z-[110] pointer-events-auto">
          <button
            type="button"
            onClick={handleEnableSound}
            disabled={enablingSound}
            className="flex items-center gap-2 rounded-2xl border border-[#CCD5AE]/35 bg-[#171A15] px-4 py-3 text-sm font-black text-[#F4F7EE] shadow-2xl transition hover:border-[#CCD5AE]/65 hover:bg-[#1E221B] disabled:opacity-60"
            title="Ativar a campainha sonora dos alertas desta tela KDS"
          >
            <Volume2 className="h-5 w-5 text-[#CCD5AE]" />
            {enablingSound ? 'Ativando campainha...' : 'Ativar campainha'}
          </button>
        </div>
      )}
