...
import { BellRing, CheckCircle2, ChefHat, Minus, Plus, Send, UtensilsCrossed, Wine } from 'lucide-react';
import { useHotel } from '../context/HotelContext.tsx';
import { KitchenOrder, MenuItem, MinibarItem, RoomMinibarConsumption } from '../types.ts';
import { api } from '../services/api.ts';

export const MinibarOperationalModule: React.FC = () => {
  ...
  const submitConsumption = async (event: React.FormEvent) => {
    ...
    if (!selectedRoomId || !selectedItemId || quantity <= 0) return;
    try {
      setSubmitting(true);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <div>
        <h2 className="text-xl sm:text-2xl font-bold text-[#2C3327] flex items-center gap-2">
          <Wine className="w-5 h-5 text-[#588157]" /> Controle de Frigobar
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <form onSubmit={submitConsumption} className="bg-white rounded-2xl border border-[#E6E3D8] p-6 shadow-xs space-y-4">
          <div>
            ...
            <select value={selectedRoomId} onChange={e => setSelectedRoomId(e.target.value)} required className="w-full px-3 py-2 text-sm border border-[#E6E3D8] rounded-xl">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-[#6B705C] mb-1">Quantidade</label>
              <input type="number" min="1" max="20" value={quantity} onChange={e => setQuantity(Number(e.target.value))} className="w-full px-3 py-2 text-sm border border-[#E6E3D8] rounded-xl" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-[#6B705C] mb-1">Registrado Por</label>
              <input value={registeredBy} onChange={e => setRegisteredBy(e.target.value)} className="w-full px-3 py-2 text-sm border border-[#E6E3D8] rounded-xl" />
            </div>
          </div>
          <button type="submit" disabled={submitting || !selectedRoomId || !selectedItemId} className="w-full py-2.5 bg-[#2C3327] text-white rounded-xl text-xs font-bold disabled:opacity-50">
            {submitting ? 'Lançando...' : 'Lançar na Conta do Quarto'}
          </button>
          ...
          <div className="pt-3 border-t border-[#E6E3D8] space-y-1.5">
            ...
            {items.map(item => (
              <div key={item.id} className="flex justify-between items-center p-2 bg-[#F4F1EA] rounded-lg text-xs">
                <span className="font-medium text-[#2C3327]">{item.name}</span>
                <span className="font-bold text-[#588157]">{item.stockQty} un</span>
              </div>
            ))}
          </div>
        </form>
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          {consumptions.length === 0 ? (
            <div className="bg-white rounded-2xl border border-[#E6E3D8] p-12 text-center text-[#8E9280] text-sm">Selecione um quarto para visualizar os lançamentos.</div>
          ) : (
            <div className="bg-white rounded-2xl border border-[#E6E3D8] overflow-x-auto shadow-xs">
              <table className="w-full text-left text-xs">
                <thead className="bg-[#F4F1EA] text-[#6B705C]"><tr><th className="p-3">Data</th><th className="p-3">Quarto</th><th className="p-3">Produto</th><th className="p-3">Qtd</th><th className="p-3">Total</th><th className="p-3">Responsável</th><th className="p-3">Status</th></tr></thead>
                <tbody className="divide-y divide-[#E6E3D8]">
                  {consumptions.map(item => <tr key={item.id}><td className="p-3">{new Date(item.registeredAt).toLocaleString('pt-BR')}</td><td className="p-3 font-bold">{item.roomNumber}</td><td className="p-3">{item.itemName}</td><td className="p-3">{item.quantity}</td><td className="p-3 font-bold text-[#588157]">{currency} {item.totalPrice.toFixed(2)}</td><td className="p-3">{item.registeredBy}</td><td className="p-3">{item.status}</td></tr>)}
                </tbody>
              </table>
            </div>
          )}
...
export const OrdersOperationalModule: React.FC<{ mode: OrdersMode }> = ({ mode }) => {
  const { rooms, settings, refreshData } = useHotel();
  ...
  useEffect(() => {
    Promise.all([api.getMenuItems(), api.getOrders()]).then(([menu, currentOrders]) => {
      setMenuItems(menu);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <div>
        <h2 className="text-xl sm:text-2xl font-bold text-[#2C3327] flex items-center gap-2">
          {mode === 'room_service' ? <BellRing className="w-5 h-5 text-[#D4A373]" /> : <ChefHat className="w-5 h-5 text-[#D4A373]" />}{title}
        </h2>
        <p className="text-xs sm:text-sm text-[#6B705C] mt-1">{subtitle}</p>
      </div>
          ...
          {visibleOrders.map(order => (
            <div key={order.id} className="bg-white rounded-2xl border border-[#E6E3D8] p-4 shadow-xs space-y-3">
              ...
              <div className="pt-2 border-t border-[#E6E3D8]">
                {order.status === 'Recebido' && <button onClick={() => updateStatus(order.id, 'Em Preparo')} className="w-full py-1.5 bg-[#D4A373] text-white font-bold rounded-lg text-xs">Iniciar Preparo</button>}
                {order.status === 'Em Preparo' && <button onClick={() => updateStatus(order.id, 'Pronto')} className="w-full py-1.5 bg-[#A3B18A] text-white font-bold rounded-lg text-xs">Pronto para Entrega</button>}
                {order.status === 'Pronto' && <button onClick={() => updateStatus(order.id, 'Entregue')} className="w-full py-1.5 bg-[#588157] text-white font-bold rounded-lg text-xs">Marcar como Entregue</button>}
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {menuItems.filter(item => item.available).map(item => {
              ...
              return <div key={item.id} className="bg-white rounded-xl border border-[#E6E3D8] p-3 flex items-center justify-between"><div><p className="text-xs font-bold text-[#2C3327]">{item.name}</p><p className="text-xs font-extrabold text-[#588157]">{currency} {item.price.toFixed(2)}</p></div><div className="flex items-center gap-1.5 bg-[#F4F1EA] p-1 rounded-lg"><button type="button" onClick={() => changeItem(item.id, -1)} disabled={quantity <= 0} className="p-1 bg-white rounded disabled:opacity-30"><Minus className="w-3 h-3" /></button><span className="w-5 text-center text-xs font-bold">{quantity}</span><button type="button" onClick={() => changeItem(item.id, 1)} className="p-1 bg-white rounded"><Plus className="w-3 h-3" /></button></div></div>;
            })}
          </div>
        </div>

        <form onSubmit={submitOrder} className="bg-white rounded-2xl border border-[#E6E3D8] p-5 shadow-xs space-y-3 h-fit">
