...
import { BedDouble, LayoutDashboard } from 'lucide-react';
...
export const RoomsTransitionWorkspace: React.FC = () => {
  const [view, setView] = useState<'operation' | 'registry'>('operation');
  ...
  return (
    <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-2xl border border-[#E6E3D8] bg-white p-3 shadow-xs">
          <div>
            ...
            <p className="text-[11px] text-[#6B705C] mt-0.5">
              O cadastro mestre já está separado. A operação antiga permanece disponível durante a transição.
            </p>
          </div>
          <div className="inline-flex bg-[#F4F1EA] p-1 rounded-xl border border-[#E6E3D8] text-xs font-semibold">
            <button
      ...
      {view === 'operation' ? <RoomsAndInventoryManager /> : <RoomsRegistryManager />}
    </div>
  );
};
